...
def entity_similarity(ep_tokens, gold_responses):
  print("resp txt vs gold responses", len(ep_tokens), len(gold_responses))
  gold_responses_tokens = list(zip(*gold_responses))
  ...
  for sent_ in gold_responses_tokens[0]:
    ...
    if len(convert_to_scy_doc(sent_)) == 0:
      ...
      #print(gold_entities)
      li_li = [most_similar_words(word_) for word_ in gold_entities]
    ...
    if are_ents == True:
      #print(ep_tokens[turn_index])
      ep_token = ep_tokens[turn_index]
      #print(ep_token)
      ...
      if math.isnan(cosine(entities_vec, ep_tokens_vec)) == True:
        entity_sim = 0
...
def most_similar_words(word):
  ...
  cosine = lambda v1, v2: dot(v1, v2) / (norm(v1) * norm(v2))
  others = list({w for w in parser.vocab if w.has_vector and w.orth_.islower() and w.lower_ != str(word_string)})
