...
class Seq2SeqModel(object):
  ...
  def step_rf(self, args, session, encoder_inputs, decoder_inputs, target_weights,
    ...
    while True:
      ...
      print(data_utils.lookup_ids(encoder_input, 5))
      ...
      if (resp_txt in self.dummy_dialogs) or (len(ep_tokens) == 5) or (encoder_inputs in ep_encoder_inputs): 
...
##### custom functions ########
  ...
  def  f_reward(self, encoder, decoder):
    ...
    if encoder.size == decoder.size:
      ...
      padd = maxx - decoder.size
      dencoder = np.concatenate((encoder, np.zeros((padd,), dtype=np.int)), axis =0)
