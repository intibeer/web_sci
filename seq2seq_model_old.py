...
from lib import seq2seq_model_utils
...
class Seq2SeqModel(object):
  ...
  def step_rf(self, args, session, encoder_inputs, decoder_inputs, target_weights,
           bucket_id, forward_only, rev_vocab=None, debug=True):
    ...
    while True:
      ...
      if (resp_txt in self.dummy_dialogs) or (len(ep_tokens) == 1) or (encoder_inputs in ep_encoder_inputs): 
    ...
    print(str(min(ep_rewards)) + " , " + str(min(ep_rewards)))
