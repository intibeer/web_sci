...
from lib import seq2seq_model_utils as seq2seq_model_utils
...
class Seq2SeqModel(object):
  ...
  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
           bucket_id, forward_only=False, training=False, force_dec_input=False, advantage=None):
  ...
  def step_rf(self, args, session, encoder_inputs, decoder_inputs, target_weights,
           bucket_id, data_, encoder_input, decoder_input, forward_only, rev_vocab=None, debug=True):
    ...
    gold_tokens, gold_txt = self.logits2tokens(decoder_inputs, rev_vocab, sent_max_length, reverse=True)
    ...
    ############ UPDATED INTI CODE ###############
    ##############################################
    #Introduced by Inti Beer
    k = args.k
    ep_tokens = []
    ep_resp = []
    ep_number = 0
    gold_responses = data_utils.lookup_ids(encoder_input, k, args.workspace)
    info_rewards = []
    ################################################
    ################################################
    ...
    while True:
      ...
      ep_resp.append(resp_txt)

      if debug: print("[RESP]: (%.4f) %s" % (step_loss, resp_txt))


      if len(enc_states) < 2:
        print("not enough encoded states")
        r = 0
        info_rewards.append(r)
      else:
        vec_a, vec_b = enc_states[-2], enc_states[-1]
        r = sum(vec_a*vec_b) / sum(abs(vec_a)*abs(vec_b))
        r = -log(abs(r))
        info_rewards.append(r)
      ...
      #bucket_id = len(resp_tokens)
      feed_data = {bucket_id: [(resp_tokens, [])]}
      encoder_inputs, decoder_inputs, target_weights, _, _, = self.get_batch(feed_data, bucket_id)
      #----------------------------------------------------

      if (resp_txt in self.dummy_dialogs) or (len(ep_tokens) == k): #or (encoder_inputs in ep_encoder_inputs): 
    ...
    def gold_rewards(gold_responses, ep_tokens):
      def clean_ids(gold_responses):
        gs = []
        arr = []
        ep_rewards = []
        for vv in gold_responses:
          gs.append(vv[1])
        for string in gs:
          stri = string.split(" ")
          stri = [int(rrr) for rrr in stri]
          arr.append(stri)
        return arr
      gold_ = clean_ids(gold_responses)

      if len(ep_tokens) != len(gold_):
        R = 0
        ep_rewards.append(R)
      else:
        for _k in range(1, k):
          turn = np.asarray(list(ep_tokens[_k]))
          gold = np.asarray(list(gold_[_k]))
          R = self.f_reward(turn, gold)
          ep_rewards.append(R)
      return ep_rewards

    #print("************EPISODE REWARDS***************")
    ep_reward_ENTITY = data_utils.entity_similarity(ep_resp, gold_responses)
    ep_reward_GOLD = gold_rewards(gold_responses, ep_tokens)
    ep_reward_INFOFLOW = info_rewards

    r1 = ep_reward_ENTITY
    r2 = ep_reward_GOLD
    r3 = ep_reward_INFOFLOW

    #Final Rewards
    print(len(ep_reward_ENTITY), len(ep_reward_GOLD), len(ep_reward_INFOFLOW))

    for er1, er2, er3 in zip(r1, r2, r3):
      ep_rewards.append(sum([er1, er2, er3])/ 3)

    ###############################################
    ###############################################

        
    # gradient decent according to batch rewards -> changed rto loss
    if max(ep_rewards) == 0:
      advantage = [0, 0, 0, 0]
    else:
      rto = (max(ep_step_loss) - min(ep_step_loss)) / (max(ep_rewards) - min(ep_rewards))
    #rto = max(ep_step_loss) / max(ep_rewards)
      advantage = [np.mean(ep_rewards)*rto] * len(args.buckets)

    print("ep_step_loss min & max:")
    print(str(min(ep_step_loss)) + " , " + str(max(ep_step_loss)))

    print("ep_rewards_min_max:")
    print(str(min(ep_rewards)) + " , " + str(max(ep_rewards)))
    ...
    #print("rto:")
    #print(rto)
    print("advantage mean:", np.mean(ep_rewards))
    ...
    ep_number += 1
    #self.write_to_file(ep_rewards, ep_number)
  ...
  def get_batch(self, data, bucket_id):
    ...
    return batch_encoder_inputs, batch_decoder_inputs, batch_weights, encoder_input, decoder_input,

##### custom functions ########
  def sigmoid(self, x):
    return 1 / (1 + math.exp(-x))

  def  f_reward(self, encoder, decoder):
    ...
    if encoder.size == decoder.size:
      ...
      #print(decoder, np.zeros((padd,), dtype=np.int))
      
    elif decoder.size == maxx:
      #print("padding encoder")
      padd = maxx - decoder.size
      encoder = np.concatenate((encoder, np.zeros((padd,), dtype=np.int)), axis =0)
      #print(encoder, np.zeros((padd,), dtype=np.int))

    encoder, decoder = set(encoder), set(decoder)
    R = nltk.f_measure(encoder, decoder)
  
    return R


  def write_to_file(self, ep_rewards, ep_number):
    ...
    with open(csvfile, "a") as output:
        ...
        #for cc in comp:
        #   writer.writerow([cc.encode('utf-8')])
        if ep_number is None:
            print("no rewards")
        else:
          pass
            #output.write("ep number:" + str(ep_number))
            #output.write('\n'.join("reward: %.4f" % ep_rewards)
            #print("reward: %.4f" % ep_rewards)
