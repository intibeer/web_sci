...
class Seq2SeqModel(object):
  ...
  def step_rf(self, args, session, encoder_inputs, decoder_inputs, target_weights,
    ...
    while b"_PAD" in query: query.remove(b"_PAD")

    symbols = ['?', '.', '!']
    query = [t.decode('unicode_escape') for t in query]
    for sym in symbols:
      for tok in query:
        if tok in sym: 
          query[query.index(tok)-1] = query[query.index(tok)-1] + query[query.index(tok)
          query.remove(tok)

    print(query)

    R = seq2seq_model_utils.reward(query, data_)
    """
    ################################################
    ################################################
    ...
    while True:
      ...
      #print(enc_states[-2])
      ############### calculate f-measure ################
      #encoder, decoder = encoder_input, decoder_input
      #print(encoder, decoder)
      #encoder = np.asarray(encoder) 
      #decoder = np.asarray(decoder)
      #R = self.f_reward(encoder, decoder)
