...
def predict(args, debug=False):
    ...
    with tf.Session() as sess, open(results_path, 'w') as results_fh:
        ...
        data = get_handles(filename)
        queries_, answers_ = zigzag(data)
        pairs_ = list(zip(queries_, answers_))

        #test_sentences = _get_test_dataset()

        for sentence in pairs_:
            # Get token-ids for the input sentence.
            qu = sentence[0]
            predicted_sentence = get_predicted_sentence(args, qu, vocab, rev_vocab, model, sess, debug=debug)

            if isinstance(predicted_sentence, list):
                for sent in predicted_sentence:
                    results_fh.write("%s \n, %s \n, %s \n" % (sentence[0], sent['dec_inp'], sentence[1]))
            else:
                results_fh.write("%s \n, %s \n, %s \n" % (sentence[0], predicted_sentence, sentence[1]))
            # break
